...
parser = argparse.ArgumentParser(description='skynet design mapped on FPGA')
parser.add_argument('--option', default='baseline', help='run baseline design or optimized design')
...
overlay = Overlay('./'+args.option+'.bit')
...
IMG_DIR = '/home/xilinx/git_px/dac_test_images/'
# Get image name list
def get_image_names():
...
#skynet_test = overlay.skynet_test_0
if args.option == 'optimized':
    skynet_test = overlay.SkyNet_0
elif args.option == 'baseline':
    skynet_test = overlay.skynet_test_0
skynet_test.write(0x10, img.physical_address)
skynet_test.write(0x18, conv_weight_1x1_all.physical_address)
skynet_test.write(0x20, conv_weight_3x3_all.physical_address)
skynet_test.write(0x28, bias_all.physical_address)
skynet_test.write(0x30, DDR_pool_3_out.physical_address)
skynet_test.write(0x38, DDR_pool_6_out.physical_address)
skynet_test.write(0x40, DDR_buf.physical_address)
skynet_test.write(0x50, predict_boxes.physical_address)
skynet_test.write(0x58, constant.physical_address)
     

rails = pynq.get_rails()
recorder = pynq.DataRecorder(rails['in1'].voltage)
...
for batch in get_image_batch():
        for i in range(0, len(batch), 4):
            ...
            skynet_test.write(0x00, 1)
            isready = skynet_test.read(0x00)
            while( isready == 1 ):
                isready = skynet_test.read(0x00)
...
print ('FPS: '+ str(1000.0/float(total_time)))
